...
class GenericAudioDataset(Dataset):
    ...
    def __init__(
        ...
        def check_file_exists(x):
            ...
            logger.info(f"Checking if file exists: {audio_path} -> {exists}")
            return exists
        ...
        logger.info(f"CSV file name: {self.csv_file_name}")
        ...
        for idx, row in self.audio_metadata.iterrows():
            ...
            try:
                ...
                logger.info(f"Audio length for file {audio_path}: {audio_length} seconds")
                num_segments = int((audio_length - self.segment_length) // (self.segment_length - self.overlap) + 1)
                logger.info(f"Number of segments for file {audio_path}: {num_segments}")
                if num_segments > 0:
...
def audio_collate_fn_segments(batch, processor):
    """
    Collate function to process a batch of audio segments.

    Args:
        batch (List[dict]): List of audio segment data.
        processor (AutoFeatureExtractor): Processor for the model.

    Returns:
        dict: Processed inputs for the model, or None if batch is empty.
    """
    ...
    logger.info(f"Audio shapes: {[audio.shape for audio in audios]}")
    inputs = processor(audios, sampling_rate=processor.sampling_rate, padding=True, return_tensors="pt")
    # Debugging: print input shapes
    logger.info(f"Input values shape after processor: {inputs['input_values'].shape}")
    inputs['id'] = ids
    return inputs
